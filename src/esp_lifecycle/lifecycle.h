...
class ESPLifecycle {
  public:
	...
	class SectionBuilder {
	  public:
		...
		SectionBuilder &
		readiness(std::function<bool()> isReady, std::function<void(TickType_t)> waitFn);
  ...
  private:
	...
	void setSectionReadiness(
	    size_t sectionIndex, std::function<bool()> isReady, std::function<void(TickType_t)> waitFn
	);

	LifecycleResult failResult(
	    LifecycleErrorCode code, const char *nodeName, const char *detail, bool updateFailedState
	);
	...
	LifecycleResult
	deinitializeInternal(const std::vector<size_t> &subset, bool updateState, bool enableParallel);
	...
	buildWavesForSubset(const std::vector<size_t> &subset, bool initializePhase) const;
