...
bool ESPLifecycle::startReloadListener(
	...
	EventBusSub subId =
	    eventBus.subscribe(eventId, [this, payloadToNodeNames](void *payload, void * /*userArg*/) {
...
void ESPLifecycle::scheduleNodeReinitialize(const std::vector<const char *> &nodeNames) {
...

	{
		...
		for (const char *name : nodeNames) {
			...
			if (std::find(pendingNodeNames.begin(), pendingNodeNames.end(), name) ==
			    pendingNodeNames.end()) {
	...
	WorkerResult spawnResult =
	    config.worker->spawn([this]() { listenerWorkerLoop(); }, workerConfig);
		...
		if (!result.ok) {
			if (result.code == LifecycleErrorCode::Busy) {
				log(LifecycleLogLevel::Warn,
				    "reload reinitialize rejected because lifecycle is busy");
			} else {
