	...
	for (size_t i = 0; i < nodes.size(); i++) {
		...
		if (nodes[i].sectionIndex >= sections.size()) {
			return failResult(
			    LifecycleErrorCode::InvalidSection,
			    nodes[i].name.c_str(),
			    "node references undefined section",
			    false
			);
		}
		...
		if (!nodes[i].initFn || !nodes[i].teardownFn) {
			return failResult(
			    LifecycleErrorCode::InvalidConfig,
			    nodes[i].name.c_str(),
			    "node requires init and teardown callbacks",
			    false
			);
		}
		...
		if (nodeByName.find(nodes[i].name) != nodeByName.end()) {
			return failResult(
			    LifecycleErrorCode::DuplicateNode,
			    nodes[i].name.c_str(),
			    "duplicate node name",
			    false
			);
	...
	for (size_t nodeIndex = 0; nodeIndex < nodes.size(); nodeIndex++) {
		...
		for (const std::string &depName : node.dependenciesByName) {
			...
			if (it == nodeByName.end()) {
				return failResult(
				    LifecycleErrorCode::MissingDependency,
				    node.name.c_str(),
				    depName.c_str(),
				    false
				);
