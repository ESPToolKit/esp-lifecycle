...
LifecycleResult ESPLifecycle::deinitialize() {
    ...
    LifecycleResult result = deinitializeInternal(
        allNodeIndexes(),
        true,
        config.enableParallelDeinit
    );
    if( !result.ok ){
...
LifecycleResult ESPLifecycle::runSectionInitialize(
    ...
    if( !sectionSubset.empty() && batches.empty() ){
        return failResult(LifecycleErrorCode::CycleDetected, nullptr, "unable to build initialize waves", true);
    }
...
LifecycleResult ESPLifecycle::deinitializeInternal(
    ...
    if( !teardownSubset.empty() && batches.empty() ){
        return failResult(LifecycleErrorCode::CycleDetected, nullptr, "unable to build deinitialize waves", true);
    }
    ...
    for( const std::vector<size_t>& batch : batches ){
        ...
        if( !enableParallel ){
            for( size_t nodeIndex : batch ){
                LifecycleResult result = initializePhase
                    ? runNodeInit(nodeIndex, true)
        ...
        if( !parallelResult.ok ){
            if( initializePhase && parallelResult.nodeName != nullptr ){
                auto it = std::find_if(nodes.begin(), nodes.end(), [&](const LifecycleNodeDefinition& node) {
                    return node.name == parallelResult.nodeName;
                });
                if( it != nodes.end() && it->optional ){
                    log(LifecycleLogLevel::Warn, "optional node init failed in parallel batch, continuing");
                    continue;
                }
            }
...
LifecycleResult ESPLifecycle::runParallelBatch(const std::vector<size_t>& batch, bool initializePhase) {
    ...
    for( size_t nodeIndex : batch ){
        ...
        if( !initializePhase && config.continueTeardownOnFailure ){
            log(LifecycleLogLevel::Warn, "teardown failed in parallel batch, continuing due to policy");
            continue;
        }
...
std::vector<std::vector<size_t>> ESPLifecycle::buildWavesForSubset(
    const std::vector<size_t>& subset,
    bool initializePhase
) const {
